...
{  
	...
	cout<< "the plaintext is : This is a message we will encrypt with AES!"<<endl;
	...
	for(int i = 0; i < lenOfPaddedMessage; i++)
	{
		...
		/*if(paddedMessage[i] / 16 < 10){
			...
			ctext[i] = ((paddedMessage[i] / 16) + '0');
		}

		if(paddedMessage[i]/ 16 >= 10){
			ctext[i]=((paddedMessage[i] / 16-10) + 'A');
		}
	...
	unsigned char cipherText[]="C0 9B 81 16 EE FB 48 8D 53 FC 4C 00 78 78 2A F6 2D 2A E0 B1 EC CF 8B 57 4E 17 DB 69 58 AE 29 18 3C D3 F7 64 50 83 02 C7 F6 31 2A 0A D1 7B 1A E6";

	int Len = strlen((const char*)cipherText);
	
	int lenOfCipherText = Len;
